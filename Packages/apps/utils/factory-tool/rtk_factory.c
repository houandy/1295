...
#define ENV_SIZE ENV_TOTAL_SIZE-4
...
#define BUSYBOX_CMD ""
#else
#define BUSYBOX_CMD "/sbin/busybox"
#endif
...
long int factory_tar_checksum(char *header)
{
...
void factory_tar_fill_checksum(char *cheader)
{
    ...
    sprintf(header->chksum, "%06o", (uint32_t)factory_tar_checksum(cheader));
...
int factory_tar_check_header(char *cheader)
{
    ...
    {
        printf("check sum error %s %o\n", header->chksum, (uint32_t)factory_tar_checksum((char *) header));
        return -1;
    }
    return 0;
...
int factory_init(void)
{
    ...
    if((gboot_type == BOOT_NAND) || (gboot_type == BOOT_EMMC) || (gboot_type == BOOT_SPI))
    {
        gfactory_size /= 2;
    }
    ...
    sprintf(cmd, "%s mkdir -p %s", BUSYBOX_CMD, factory_dir);
    ...
    if(gerasesize == 0) {
        install_fail("erase size is zero\n");
        return -1;
    }
...
int factory_load_ro()
{
    ...
    sprintf(cmd, "cd /;%s dd if=%s bs=%u count=%u skip=%u | %s tar x", BUSYBOX_CMD, gdev, gerasesize,
            (tarsize + gerasesize - 1) / gerasesize, gfactory_ro_start / gerasesize, BUSYBOX_CMD);
...
int factory_load(void)
{
    ...
    sprintf(cmd, "%s rm -rf %s; %s mkdir -p %s", 
            BUSYBOX_CMD, DEFACULT_FACTORY_DIR, BUSYBOX_CMD, DEFACULT_FACTORY_DIR);

redo:
    // read factory tarfile
    sprintf(cmd, "cd /;%s dd if=%s bs=%u count=%u skip=%u | %s tar x", BUSYBOX_CMD, gdev, gerasesize, gfactory_size / gerasesize,
            (gfactory_start + current_pp * gfactory_block_size) / gerasesize, BUSYBOX_CMD);
    ...
    if(ret < 0)
    {
        ...
        sprintf(cmd, "cd /;%s dd if=%s bs=%u count=%u skip=%u > /cache/bad_fac.tar", BUSYBOX_CMD, gdev, 512, (tarsize + 511) / 512,
    ...
    else
    {
        ...
        if (retry)
        {
            ...
            sprintf(cmd, "cd /;%s dd if=/dev/zero of=/tmp/zero.bin bs=%u count=%u seek=%u conv=notrunc,sync",
                    BUSYBOX_CMD,
...
int spi_factory_flush(unsigned int factory_start, unsigned int factory_size)
{
    ...
    file_size = (uint32_t)rtk_get_size_of_file(FACTORY_FILE_PATH);
    //printf("file_size = %d\n", file_size);
...
int factory_flush(unsigned int factory_start, unsigned int factory_size)
{
    ...
    sprintf(cmd, "%s tar cvf %s %s", BUSYBOX_CMD, FACTORY_FILE_PATH, factory_dir);
#else
    sprintf(cmd, "%s tar cf %s %s", BUSYBOX_CMD, FACTORY_FILE_PATH, factory_dir);
#endif
    ret = rtk_command(cmd, __LINE__, __FILE__);
    if(ret < 0)
    {
