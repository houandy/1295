...
ifeq ($(strip $(CONFIG_EXTERNAL_KERNEL_TREE)),"")
  ifeq ($(strip $(CONFIG_KERNEL_GIT_CLONE_URI)),"")
    ifneq ($(strip $(CONFIG_EXTERNAL_RTK_SDK_KERNEL_TREE)),"")
	define Kernel/Prepare/Default
		...
		mkdir -p $(LINUX_DIR)
		if [ -d  $(CONFIG_EXTERNAL_RTK_SDK_KERNEL_TREE) ]; then \
		cp $(CONFIG_EXTERNAL_RTK_SDK_KERNEL_TREE)/* $(LINUX_DIR)/ -a; \
		fi
		if [ -d $(TOPDIR)/$(CONFIG_EXTERNAL_RTK_SDK_KERNEL_TREE) ]; then \
		cp $(TOPDIR)/$(CONFIG_EXTERNAL_RTK_SDK_KERNEL_TREE)/* $(LINUX_DIR)/ -a; \
		fi;
		$(Kernel/Patch)
  ...
  else ifneq (,$(CONFIG_TARGET_realtek))
    define Kernel/Prepare/Default
	xzcat $(DL_DIR)/$(LINUX_SOURCE) | $(TAR) -C $(KERNEL_BUILD_DIR) $(TAR_OPTIONS)
	$(Kernel/Patch)
	$(if $(QUILT),touch $(LINUX_DIR)/.quilt_used)
  ...
  ifneq (,$(CONFIG_TARGET_realtek))
	...
	mkdir -p $(LINUX_DIR)
	cp $(TOPDIR)/$(CONFIG_EXTERNAL_KERNEL_TREE)/* $(LINUX_DIR)/ -a	
	$(Kernel/Patch)
	$(if $(QUILT),touch $(LINUX_DIR)/.quilt_used)
     endef
  else
...
ifeq ($(CONFIG_TARGET_ROOTFS_INITRD),y)
	...
	echo 'CONFIG_ACPI_TABLE_UPGRADE=y' >> $(LINUX_DIR)/.config
    endef
endif
endif

ifeq ($(CONFIG_TARGET_ROOTFS_INITRD),y)
define Kernel/SetInitramfs/PostConfigure
...
endef
  ...
  define Kernel/SetInitramfs
	...
	$(call Kernel/SetInitramfs/PostConfigure)
  endef
