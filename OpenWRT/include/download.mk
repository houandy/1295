...
# args: $(SUBDIR) $(VERSION)
define record_scm_version
	$(if $(1),(cd $(1) && $(SCRIPT_DIR)/setlocalversion --save-scmversion),false)
endef
...
define DownloadMethod/svn
	$(call wrap_mirror,$(1),$(2), \
		...
		echo "Packing checkout..." && \
		$(call record_scm_version,$(SUBDIR)) && \
...
define DownloadMethod/gits
	...
	for path in $(URL_PATHS); do \
		git clone $(OPTS) $(URL)/$$$$path && \
		(cd $$$$path && git checkout $(VERSION) && \
		git submodule update --init --recursive) && \
		$(call record_scm_version,$$$$path) && \
		rm -rf $$$$path/.git || \
		{ echo "Failed to clone $$$$path"; exit 1; }; \
	done; \
	$(if $(PREPACK_CMD),$(PREPACK_CMD);) \
	) && \
	echo "Packing checkout..." && \
	export TAR_TIMESTAMP="" && \
	$(call dl_tar_pack,$(TMP_DIR)/dl/$(FILE),$(SUBDIR)) && \
...
define Download/default
  FILE:=$(PKG_SOURCE)
  URL:=$(PKG_SOURCE_URL)
  $(if $(PKG_SOURCE_URL_PATHS),URL_PATHS:=$(PKG_SOURCE_URL_PATHS))
  ...
  $(if $(PKG_SOURCE_PREPACK_CMD),PREPACK_CMD:=$(PKG_SOURCE_PREPACK_CMD))
